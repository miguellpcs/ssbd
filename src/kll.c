...
KLL *init();
void free_sketch(KLL *);

KLL *update(KLL *, int32_t);
uint32_t rank(KLL *);
int32_t quantile(KLL *, double);
...
void print_help()
{
    ...
    printf("--delta error_prob: ");
    printf("Especifica o valor do parametro δ do sketch.\n\n");
    printf("--mult multiplier: ");
    printf("Especifica o valor da constante multiplicadora 0.5 < C < 1 usada no sketch.\n\n");
    printf("--univ universe_size: ");
    printf("Especifica o valor do parametro U do sketch. Ou seja, consideramos os valores contidos no intervalo [U] = {0, ..., U - 1}. Os valores fora desse intervalo devem ser desconsiderados. Este campo é opcional\n\n");
...
int main(int argc, const char *argv[])
{
    ...
    int status = opt_init(&opt, "val:eps:delta:mult:univ:help:rank:quant:in:", argc, argv);
    ...
    while (get_opt(opt, &opt_key, &opt_args) == OPT_SUCCESS)
    {
        ...
        {
            error_prob = atof(opt_args[0]);
        }
        else if (strcmp(opt_key, "mult") == 0)
        {
            mult = atof(opt_args[0]);
        }
    ...
    const char *filename = strdup(argv[9]);
...
KLL *init()
{
    KLL *sketch = check_calloc(1, sizeof(KLL));
    sketch->values = check_calloc(1, sizeof(int32_t));
    return sketch;
}

void free_sketch(KLL *sketch)
{
    check_free(sketch->values);
    check_free(sketch);
}

KLL *update(KLL *sketch, int32_t value)
{
    return sketch;
}

uint32_t rank(KLL *sketch)
{
    return 0;
}

int32_t quantile(KLL *sketch, double q)
{
    return 0;
}